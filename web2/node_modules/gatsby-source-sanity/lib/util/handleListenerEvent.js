...
function handleListenerEvent(event, publishedNodes, args, processingOptions) {
    const { actions, createNodeId, getNode } = args;
    const { createNode, deleteNode } = actions;
    const { overlayDrafts } = processingOptions;
    const current = getNode(documentIds_1.safeId(documentIds_1.unprefixId(event.documentId), createNodeId));
    let published = publishedNodes.get(documentIds_1.unprefixId(event.documentId));
    if (published) {
        published = removeGatsbyInternalProps_1.removeGatsbyInternalProps(published);
    }
    const touchedIsDraft = documentIds_1.isDraftId(event.documentId);
    const currentIsDraft = current && documentIds_1.isDraftId(current._id);
    ...
    if (event.transition === 'disappear') {
        ...
        if (touchedIsDraft && event.result) {
            ...
            publishedNodes.set(event.documentId, normalize_1.processDocument(event.result, Object.assign(Object.assign({}, processingOptions), { skipCreate: true })));
        }
